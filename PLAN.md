...
11. 네이버 웹문서·뉴스·블로그 보완 검색과 Tavily 상위 본문 추출을 선택형 서버 Secret으로 연결하고, 키가 없을 때 기존 검색 경로를 유지.
12. 의미 기반 전형 색상, 준비 체크리스트·앱 내부 리마인더·즐겨찾기, 캘린더 빠른 보기·밀도·ICS, URL 분석 미리보기와 기존 공고 원문 보완을 기존 D1 상태에 하위 호환 필드로 연결한 뒤 회귀 검증·Workers 배포.
