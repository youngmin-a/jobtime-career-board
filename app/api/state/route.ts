...
export async function POST(request:Request){
 if(!viewer(request).canEdit)return Response.json({error:'이 보드는 소유자만 수정할 수 있습니다.'},{status:403});
 try{
  ...
  if(input.action==='save-selected'){
   ...
   const draft:Application=input.application;validateApplication(draft);
  ...
  const state=await updateStore(input.revision,state=>{
   ...
   if(existing){a.origin=existing.origin;a.sourceName=existing.sourceName;a.provider=existing.provider;a.officialPostingId=existing.officialPostingId;a.evidence=existing.evidence;a.officialCheckedAt=existing.officialCheckedAt;a.officialRecruitmentSnapshot=existing.officialRecruitmentSnapshot;a.createdAt=existing.createdAt;a.recruitmentOrigin=existing.origin!=="manual"?(JSON.stringify(a.recruitment)===JSON.stringify(existing.officialRecruitmentSnapshot)?existing.origin:"user_override"):"manual"}
   else if(!official){a.origin="manual";a.sourceName=undefined;a.provider=null;a.officialPostingId=null;a.evidence=null;a.officialCheckedAt=null;a.officialRecruitmentSnapshot=null;a.recruitmentOrigin="manual";a.createdAt=new Date().toISOString()}
