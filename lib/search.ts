...
export type SearchConfig={braveApiKey?:string;alioApiKey?:string;alioApiUrl?:string};
...
export async function searchJobs(query:string,page=1,config:SearchConfig={}):Promise<SearchResult>{
 ...
 if(page===1&&config.braveApiKey){try{webCandidates=await externalWebSearch(queryText,config.braveApiKey)}catch{webWarnings.push('웹 검색 제공자 조회에 실패했습니다. 공식·공개 결과는 유지됩니다.')}}
 if(page===1&&config.alioApiKey&&config.alioApiUrl){try{webCandidates=[...(webCandidates??[]),...(await alioSearch(queryText,config.alioApiKey,config.alioApiUrl))]}catch{webWarnings.push('ALIO 조회에 실패했습니다. 공식·공개 결과는 유지됩니다.')}}
 ...
 return {query:queryText,candidates:unique,uncertainCandidates:uncertain,webCandidates,warnings,total,scanned:candidates.length,partial:partial||hasMore,failed:success===0,nextPage:hasMore&&page<MAX_PAGES?page+1:null,manualUrl};
}
